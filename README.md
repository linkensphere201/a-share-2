...
The script always uses `build\desktop` for PyInstaller work files and replaces the single package at `dist\StockHarness\StockHarness.exe`; it does not create version-suffixed output directories. Before building, it force-stops only `StockHarness.exe` processes whose executable path is inside that fixed package directory, waits for them to exit, and aborts if the package remains locked. The application starts the local FastAPI service on stable loopback origin `http://127.0.0.1:8765`, serves the production React build from that origin, opens a WebView2 window, and stops the service when the window closes. WebView2 runs with a persistent profile under `%LOCALAPPDATA%\StockHarness\WebView`, so workspace layouts survive application restarts. WebView2 must be available on the machine.
