...
When futures are enabled, APP startup runs the same final-data updater as stocks. It refreshes catalog and calendars, checks the configured final-data cutoff, reads only missing tails plus the configured correction window, and updates affected mappings. Exchange calendars extend 14 calendar days beyond the completed-bar cutoff so a Friday night session can resolve the next open Monday; final bars and mappings remain bounded to completed trading days. Final bars are requested once per exchange and open trading day, then distributed to the applicable contract cursors. Main-contract mappings are requested once per trading day and reused across exchange passes. A failed daily batch does not advance affected cursors; a failed mapping batch falls back to the bounded per-series path. A no-change run does not rewrite history.
