...
export function LayoutManager({ workspace, onChange, onClose }: LayoutManagerProps) {
  ...
  const addWindow = (type: 'instrument-list' | 'chart', mode: ChartWindowState['mode']) => {
    ...
    const added: WorkspaceWindowState = type === 'chart'
      ? { id, type: 'chart', title, mode, presentation: { mode: 'docked' }, instrument, chart: { range: '3Y', priceMode: 'normal', volumeVisible: true, indicator: 'macd', settlementVisible: false, openInterestVisible: false, drawingToolbarCollapsed: false, tradingSystems: createTradingSystemWindowStates() } }
